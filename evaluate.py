...
IMAGE_ENCODER_DIR = "../outputs/train_image_encoder/"
MASK_ENCODER_DIR = "../outputs/train_mask_encoder/"
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")


if __name__ == '__main__':
    mask_encoder = models.MaskEncoder().to(DEVICE)
    image_encoder = models.ImageEncoder().to(DEVICE)
    image_decoder = models.ImageDecoder().to(DEVICE)

    mask_encoder.load_state_dict(torch.load(MASK_ENCODER_DIR + "mask_encoder.params"))
    ...
    r_mask = 1.0 - mask
    mask_features = mask_encoder(mask)
    ...
    for x in range(0, 100):
        ...
        fake_image = image_decoder(image_features, mask_features)
